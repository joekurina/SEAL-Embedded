 ...
 * @file ckks_common.h
 * 
 * Common definitions and functions for CKKS encryption that can be used in both C and C++ code.
 */
  ...
  */
 typedef struct SE_PTRS_t
 {
     complex_double* conj_vals;     // Storage for output of encode
     complex_double* ifft_roots;    // Roots for inverse fft (n double complex values)
 ...
 bool ckks_encode_base(const Parms* parms, const flpt* values, size_t values_len,
                        uint16_t* index_map, complex_double* ifft_roots, 
                        complex_double* conj_vals);
  ...
  */
 void se_print_relative_positions(const ZZ* st, const SE_PTRS* se_ptrs, size_t n, int sym);
  ...
  * @param sym      Set to 1 if in symmetric mode
  */
 void se_print_addresses(const ZZ* mempool, const SE_PTRS* se_ptrs, size_t n, int sym);
  ...
  * @param sym      Set to 1 if in symmetric mode
  */
 void print_ckks_mempool_size(size_t n, int sym);
 #else
 /**
  * Prints the relative positions of various objects.
  * 
