...
public:
    ...
    void operator()(sycl::handler& h) const {
        ...
        const uint32_t* kernel_const_ratio = const_ratio; // Pointer copy is fine
        ...
        h.single_task([=]() [[intel::kernel_args_restrict]] {
            ...
            while (i < kernel_n) {
                // Non-blocking read from the input pipe:
                int64_t val;
                bool read_succeeded = false; 
                val = ScaleToReducePipe::read(read_succeeded);

                // Perform operations only if read was successful
                if (read_succeeded) {
                    ...
                    // Increment index for each successful read
                    i++;
                } else {
                    // Read Failed, retry the read operation
                }
            } // End of while loop
...
}; // End of ReduceSetPTEKernel class