/**
 * @file ckks_tests_common.cpp
 * C++ implementation of common CKKS testing functionality for SEAL-Embedded
 */
 ...
 extern "C" void set_encode_encrypt_test(size_t testnum, size_t vlen, flpt *v)
 {
     se_assert(v);
     clear_flpt(v, vlen);
     ...
     switch (testnum)
     {
         ...
         case 4: set_flpt(v, vlen, static_cast<flpt>(1.1)); break;
         case 5: set_flpt(v, vlen, static_cast<flpt>(-2.1)); break;
         case 6:
             set_flpt(v, vlen, static_cast<flpt>(1));
 ...
 extern "C" void ckks_decode(const ZZ *pt, size_t values_len, uint16_t *index_map, const Parms *parms,
                  double complex *temp, flpt *values_decoded)
 {
     ...
     // Create a properly typed pointer for C++ complex use
     std::complex<double>* res = reinterpret_cast<std::complex<double>*>(temp);
     ...
     for (size_t i = 0; i < n; i++)
     {
         ...
         ZZ val      = pt[i];
         double dval = (val > q / 2) ? -(static_cast<double>(q - val)) : (static_cast<double>(val));
 
         // -- We scale by 1/scale here
         res[i] = std::complex<double>(dval / scale, 0.0);
     }
     
     print_poly_double_complex("res           ", temp, n);
 
     // Call FFT with properly typed pointers
     fft_inpl(reinterpret_cast<fft_complex*>(res), n, logn, nullptr);
     ...
     double *res_double = reinterpret_cast<double*>(res);
     for (size_t i = 0; i < n; i++) { 
         res_double[i] = res[i].real(); 
     }
     print_poly_double("res double    ", res_double, n);
 
     uint16_t *index_map_ = reinterpret_cast<uint16_t*>(&(res[n / 2]));
     ckks_calc_index_map(parms, index_map_);
 
     for (size_t i = 0; i < values_len; i++) { 
         values_decoded[i] = static_cast<flpt>(res_double[index_map_[i]]); 
     }
 #else
     se_assert(index_map);
 ...
 #endif
     for (size_t i = 0; i < values_len; i++) { 
         values_decoded[i] = static_cast<flpt>(res[index_map[i]].real()); 
     }
 #endif
 }
 
 extern "C" void check_decode_inpl(ZZ *pt, const flpt *values, size_t values_len, uint16_t *index_map,
                        const Parms *parms, ZZ *temp)
 {
     ...
     ckks_decode_inpl(pt, values_len, index_map, parms, reinterpret_cast<double complex*>(temp));
     print_poly_flpt(n2, reinterpret_cast<flpt*>(pt), n);
     bool err = compare_poly_flpt(n1, values, n2, reinterpret_cast<flpt*>(pt), values_len, static_cast<flpt>(0.1));
     se_assert(!err);
 }
 
 extern "C" void ckks_decrypt(const ZZ *c0, const ZZ *c1, const ZZ *s, bool small_s, const Parms *parms, ZZ *pt)
 {
 ...
 extern "C" void ckks_decrypt_inpl(ZZ *c0, ZZ *c1, const ZZ *s, bool small_s, const Parms *parms)
 {
     // -- c0 = [-a*s + e + pt]_Rq ; c1 = a
 ...
 extern "C" void check_decode_decrypt_inpl(ZZ *c0, ZZ *c1, const flpt *values, size_t values_len, const ZZ *s,
                                ...
                                const Parms *parms, ZZ *temp)
 {
     ...
     */
     ...
     if (values)
     {
         ...
         const char *n1 = "values        ";
         const char *n2 = "values_decoded";
         ckks_decode_inpl(c0, values_len, index_map, parms, reinterpret_cast<double complex*>(temp));
         print_poly_flpt(n2, reinterpret_cast<flpt*>(c0), n);
         bool err = compare_poly_flpt(n1, values, n2, reinterpret_cast<flpt*>(c0), values_len, static_cast<flpt>(0.1));
         se_assert(!err);
     }
 }