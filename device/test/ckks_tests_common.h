 ...
 * Common testing functions for CKKS implementation that work in both C and C++
 */

 #pragma once

 // Include complex types depending on language
 #ifdef __cplusplus
 #include <complex>
 ...
 void ckks_decode(const ZZ *pt, size_t values_len, uint16_t *index_map, const Parms *parms,
                  complex_double *temp, flpt *values_decoded);
 ...
 static inline void ckks_decode_inpl(ZZ *pt, size_t values_len, uint16_t *index_map,
                                     const Parms *parms, complex_double *temp)
 {
